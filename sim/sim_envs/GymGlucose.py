from sim_envs.env import T1DSimEnv as _T1DSimEnv
...
class T1DSimEnv(gym.Env):
    ...
    def action_space(self):
        """
        The amount of insulin for the pump to administer when `step()`
        is called.

            basal = u2ss (pmol/(L*kg)) * body_weight (kg) / 6000 (U/min)       
        Define the action space to be one of the following:

        
        * spaces.Box : continuous action space of range [a,b].
        
        ** Set the upper bound ub of the basal equal to the maximum basal
        of the pump. The agent must select a continuous action from the
        range (0,ub)
        

        * spaces.Discrete : discretized action space of range [a,n,b),
